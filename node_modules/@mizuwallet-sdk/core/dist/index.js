...
import { Network as c } from "@aptos-labs/ts-sdk";
import n, { gql as i } from "graphql-request";
...
}, N = "https://mizu.io/keyless_google";
class h {
  /**
  ...
  constructor(e) {
    ...
    this.appId = e.appId, this.network = e.network, this.graphqlEndPoint = w[this.network], this.initialized = !0;
  }
  /**
   ...
   */
  updateNetwork(e) {
    this.checkInitialized(), this.network = e, this.graphqlEndPoint = w[this.network];
  }
  /**
   * Get network info
   */
  get networkInfo() {
    return {
      name: this.network,
      chainId: this.network === c.MAINNET ? 1 : 2,
      url: this.network === c.MAINNET ? "https://fullnode.mainnet.aptoslabs.com/v1" : "https://fullnode.testnet.aptoslabs.com/v1"
    };
  }
  /**
   *
  ...
  async loginInTG(e, t) {
    ...
    try {
      [this.userId, this.jwtToken] = h.decodeJWTToken(a);
    } catch {
      this.logout();
    }
  }
  ...
  async isUserExistByTgID(e) {
    ...
    return ((a = t == null ? void 0 : t.telegramUser) == null ? void 0 : a.length) > 0;
  }
  /**
  ...
  async getUserWalletAddress() {
    return this.checkInitialized(), this.checkJWTToken(), (await n({
      url: this.graphqlEndPoint,
      document: p,
      variables: {
        id: this.userId
  ...
  async startBindGoogle(e) {
    ...
    window.open(`${N}?${t.toString()}`, "_blank");
  }
  /**
   *
  ...
  async createOrder(e) {
    ...
    const t = await n({
      ...
      variables: {
        appId: this.appId,
        payload: window.btoa(JSON.stringify(I(e.payload)))
      },
      requestHeaders: {
        Authorization: `Bearer ${this.jwtToken}`
      }
  ...
  async waitForOrder(e) {
    ...
    for (; a-- > 0 && ![d.SUCCESS, d.FAIL, d.CANCELED].includes(t.status); )
  ...
  async fetchOrderList(e) {
    this.checkInitialized(), this.checkJWTToken();
    const { limit: t = 10, offset: a = 0 } = e || {}, o = await n({
      url: this.graphqlEndPoint,
      document: L,
      variables: {
    ...
    return {
      data: o == null ? void 0 : o.order.map((u) => {
        let l = {};
        try {
          l = JSON.parse(window.atob(u.payload));
        } catch (g) {
          console.error(g);
        }
        return {
